...
| 🔴 红 | 需绕登录风控/反爬,或 ToS 明确禁止(银行、券商、社交平台私有接口) | **拒绝** + 降级话术(§2.6) |

### 2.5 不接清单(红灯,直接拒)
银行 / 券商 / 微信微博抖音等社交平台私有接口 / 任何需要模拟登录绕过风控、破解反爬、违反 ToS 的源。理由对用户直说:"这个源在我的不接清单上——要绕登录风控,不安全也不合规。"
