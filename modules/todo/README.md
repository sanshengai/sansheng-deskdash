...
双击时两个 UP 各起一个进程,均**只追加(`O_APPEND`)+ 只读计数**(不 truncate),避开 read-modify-write 竞争(`O_APPEND` 在 Win 上非内核级原子,详见下「并发语义」的丢写说明)。旧点击由 **`collector.prune_clicks()` 每轮采集顺手剪掉**(留 `CLICKS_TTL=5s` 内)——orchestrator 周期只跑 `collector.py`,band 从不触发 `todo_action render`,故回收必须挂在采集这条必经路上,否则 `.clicks` 只增不减。
...
| `.clicks` | `O_APPEND` 单次小写 + 只读计数 | 避开 read-modify-write 的 truncate 竞争;本场景(每次一行、毫秒级间隔)实践安全 | **非**内核级原子写:CPython 在 Win 上 `O_APPEND`=CRT seek-to-end+write 两步,高并发下理论上丢写(一次 UP 覆盖另一次 → 双击被少计成单击)——**勿当硬原子保证**,高并发模块另设计 |
